...
model: 'GPT-5.3-Codex'
tools: [vscode/memory, vscode/runCommand, execute, read, agent, edit, search, web]
...
- Store all scraped or fetched files under `./artifacts` in the project root.
- Do not use the system `/tmp` directory; rely on `./artifacts/tmp` for temp data.
